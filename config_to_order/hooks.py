...
fixtures = [
	{
		"doctype": "DocType",
		"filters": [
			["name", "in", ["Configuration Result", "Configuration Result Item", "computer"]]
		],
	},
	{
...
doc_events = {
	"BOM": {"validate": "config_to_order.doc_events.bom.validate"},
	"Sales Order": {
...
def _patch():
	...
	def get_bom_items_as_dict(
		...
		for item, item_details in item_dict.items():
			for d in [
				...
				if not item_details.get(d[1]) or (
					company_in_record and company != company_in_record
				):
					item_dict[item][d[1]] = (
						frappe.get_cached_value("Company", company, d[2]) if d[2] else None
					)
	...
	def get_child_exploded_items(self, bom_no, stock_qty):
		...
		child_fb_items = frappe.db.sql(
			"""select bom_item.item_code, bom_item.item_name,
				...
				where bom_item.parent = bom.name and bom.name = %s and bom.docstatus = 1""",
			bom_no,
			as_dict=1,
		)
	...
	def set_required_items(self, reset_only_qty=False):
		...
		if self.bom_no and self.qty:
			...
			soi = (
				frappe.get_doc("Sales Order Item", self.sales_order_item)
				if self.sales_order_item
				else None
			)
			configuration = (
				frappe.get_doc(soi.configuration_doctype, soi.configuration_docname)
				if soi and soi.configuration_docname
				else None
			)
					...
					if check_selection_condition(configuration, item):
						self.append(
							"required_items",
							{
								...
								"description": get_qty_or_desc(
									configuration, item, "description", "desc_from_configuration"
								),
								"required_qty": get_qty_or_desc(
									configuration, item, "qty", "qty_from_configuration"
								),
